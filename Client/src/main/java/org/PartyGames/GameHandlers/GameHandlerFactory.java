...
public class GameHandlerFactory {
    ...
    public static GameHandler createGameHandler(Games game, TerminalIOHandler io_handler, WebSocketConnectionHandler connection) {
        switch (game) {
            case Games.Null -> {
                logger.warn("Used factory to create NullGame, Beware!");
                return new NullGame(io_handler, connection);
            }
            case Games.Lobby -> {
                return new Lobby(io_handler, connection);
            }
            default -> {
